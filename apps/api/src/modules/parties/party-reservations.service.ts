...
import { computePartyQuote, formatMoney, moneyUnits, nonBlank, nonNegativeInteger, nonNegativeMoney, positiveMoney } from './parties.pricing.js';
...
export class PartyReservationsService {
  ...
  public async updateReservation(
    ...
    return this.repository.transaction(async (client) => {
      ...
      if (roomOrTimeChanged) {
        ...
        if (room?.branchId !== current.branchId)
          throw new PartyError('resource_not_found', 'The room was not found in this branch.');
      ...
      if (recomputeQuote) {
        ...
        const quote = computePartyQuote({
          ...
          children: childrenCount,
          adults: input.adultsCount === undefined ? 0 : nonNegativeInteger(input.adultsCount, 'adults_count'),
  ...
  public async addSnack(
    ...
    return this.repository.transaction(async (client) => {
      ...
      const lineTotalUnits = (moneyUnits(unitPriceSnapshot) * BigInt(Math.round(Number(input.quantity) * 1_000_000))) / 1_000_000n;
  ...
  public async balance(
    ...
    branchIds: readonly string[],
    reservationId: string,
  ): Promise<{ quotedTotal: string; totalPaid: string; outstandingBalance: string }> {
...
function renderPartyDocumentHtml(
  ...
  const title = documentType === 'contract' ? 'Contrato de Servicios' : 'Deslinde de Responsabilidad';
  const businessName = organization === null ? 'Business' : organization.companyName;
  const branchName = organization === null ? '' : organization.branchName;
  const customerName = reservation.customerDisplayName ?? 'Walk-in';
  const celebrant = reservation.celebrantName ?? 'N/A';
  const clauses =
    documentType === 'contract'
      ? [
          'The customer agrees to the event date, time, room, and package described above.',
          'A deposit secures the reservation; the remaining balance is due on or before the event date.',
          'Cancellations are subject to this business\'s posted cancellation policy; any refund of amounts already paid is handled separately by a manager, never automatically.',
          'The business is not responsible for personal items lost or damaged during the event.',
          'Any additional guests, snacks, or extras beyond what is described above are billed as additional charges at the time of service.',
        ]
      : [
          'The undersigned acknowledges the inherent risks of trampoline/play-area activities for all attendees.',
          'The undersigned releases the business from liability for injuries arising from ordinary use of the facilities, except in cases of gross negligence.',
          'The undersigned agrees all attendees will follow posted safety rules and staff instructions at all times.',
        ];
...
<h2>${escapeHtml(title)} - Reservation ${escapeHtml(reservation.reservationNumber)}</h2>
<table>
<tr><th>Customer</th><td>${escapeHtml(customerName)}</td><th>Celebrant</th><td>${escapeHtml(celebrant)}${
    reservation.celebrantAge === null ? '' : ` (age ${String(reservation.celebrantAge)})`
  }</td></tr>
<tr><th>Event date</th><td>${escapeHtml(reservation.eventDate)}</td><th>Time</th><td>${escapeHtml(reservation.startTime)} - ${escapeHtml(reservation.endTime)}</td></tr>
<tr><th>Room</th><td>${escapeHtml(room?.name ?? 'N/A')}</td><th>Package</th><td>${escapeHtml(pkg?.name ?? 'N/A')}</td></tr>
<tr><th>Children</th><td>${String(reservation.childrenCount)}</td><th>Total</th><td class="total">${escapeHtml(reservation.currencyCode)} ${escapeHtml(reservation.quotedTotal)}</td></tr>
</table>
${reservation.notes === null ? '' : `<p><strong>Notes:</strong> ${escapeHtml(reservation.notes)}</p>`}
<ol>${clauseItems}</ol>
