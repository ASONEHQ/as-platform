...
/** TASK 16.20A (Part 7-9) — the snack mirror of `party-sock-deduction.ts`'s
 * `postPartySockCorrection`; see that function's own doc comment for the
 * full rationale (delta-only compensating movement, `return` for a
 * downward correction, another `issue` for an upward one, old movements
 * always left immutable). */
export async function postPartySnackCorrection(
  client: PartySnackDeductionTransaction,
  context: PartySnackDeductionContext,
  reservation: PartySnackDeductionReservation,
  snack: { id: string; productVariantId: string; nameSnapshot: string },
  ...
  if (variant === undefined)
    throw new PartySnackDeductionError('inventory_location_not_found', 'The snack has no valid inventory variant.');
  ...
  const isReturn = deltaQuantity.startsWith('-');
  const movementType = isReturn ? 'return' : 'issue';
  const magnitudeUnits = decimalUnits(isReturn ? deltaQuantity.slice(1) : deltaQuantity);
  if (magnitudeUnits === 0n)
    throw new PartySnackDeductionError('insufficient_inventory', 'There is nothing to correct.');
  ...
  await client.query(
    `insert into inventory_movements
     ...
     values ($1,$2,$3,$4,$5,'posted','party_reservation_snack',$6,$7,1,$8,$8,$9,$9,$8,$8)`,
    [
      movementId,
      context.companyId,
  ...
  if (balance === undefined || (!isReturn && (next < 0n || next < reserved)))
    throw new PartySnackDeductionError(
      ...
      `Available inventory is insufficient to correct snack "${snack.nameSnapshot}".`,
    );
  ...
  await client.query(
    ...
    [
      ...
      JSON.stringify({
        ...
        correction_delta: deltaQuantity,
      }),
      context.timestamp,
    ],
  );
