...
export class PurchasingService {
  public constructor(private readonly repository: PurchasingRepository) {}
   ...
   * BigInt-scaled money arithmetic; (2) post the real `receipt` inventory
   * movement via `postDirectPurchaseReceipt` (which itself validates the
   * product variant/branch location and mutates `inventory_balances`);
   * (3) insert the `direct_purchases` row referencing the movement's own
   * id; (4) audit + outbox. A failure at any point rolls every one of
   * these back together — see `postDirectPurchaseReceipt`'s own doc
   * comment for why an invalid `product_variant_id` can never leave an
   * orphaned movement OR an orphaned direct-purchase row.
   */
  public async recordDirectPurchase(
    ...
    const cleanSupplier = nonBlankOptional(input.supplierName, 'supplier_name', 500);
