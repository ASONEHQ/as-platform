...
export const INVENTORY_QUANTITY_MAX = 9_999_999_999_999n * SCALE + 999_999n;
export function inventoryDecimal(value: string): bigint {
  ...
  if (result > INVENTORY_QUANTITY_MAX)
    throw new InventoryDraftError('numeric_overflow', 'A quantity exceeds range.');
  return result;
}
export function formatInventoryDecimal(value: bigint): string {
  const sign = value < 0n ? '-' : '';
...
export function inventoryBalanceKey(value: BalanceKey): string {
  return `${value.companyId}:${value.branchId}:${value.locationId}:${value.variantId}`;
}
export function sortInventoryBalanceKeys(values: readonly BalanceKey[]): BalanceKey[] {
  return [...values].sort((a, b) => inventoryBalanceKey(a).localeCompare(inventoryBalanceKey(b)));
}
...
export class InventoryPostingService {
    ...
    return this.repository.transaction((client) =>
      this.repository.drafts.idempotent(
        ...
        async () => {
          ...
          const existingKeys = new Set(existing.map((value) => inventoryBalanceKey(value)));
          const inbound = keys.filter(
            (value) =>
              !existingKeys.has(inventoryBalanceKey(value)) &&
              (deltas.get(inventoryBalanceKey(value))?.delta ?? 0n) > 0n,
          );
          ...
          for (const balanceKey of keys) {
            const delta = deltas.get(inventoryBalanceKey(balanceKey))?.delta ?? 0n;
  ...
  private aggregate(
    ...
    for (const line of lines) {
      ...
      const current = values.get(inventoryBalanceKey(balanceKey));
      values.set(inventoryBalanceKey(balanceKey), {
        balanceKey,
...
export function inventoryDecimalNonnegative(value: string): bigint {
  if (value.startsWith('-'))
