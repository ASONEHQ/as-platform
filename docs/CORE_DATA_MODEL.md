...
Customer identity, membership plans/entitlements, and the AS Rewards+ loyalty-ledger foundation are no longer excluded — TASK 13.0 approved and built them (`customers`, `customer_qr_tokens`, `membership_plans`, `membership_plan_branches`, `customer_memberships`, `loyalty_programs`, `loyalty_accounts`, `loyalty_ledger`; see ADR-0017 and `docs/API_CONTRACTS.md` §16.10). Exactly as this section already anticipated, they own their own schema/migrations rather than being folded into this document's 32-entity core count. `sales.customer_id`/`sales.customer_display_name` are additive columns on the existing `sales` table (ADR-0017 D6) — the two exact deviations this section's own principle above requires an ADR for. TASK 13.1 built the reward-entitlement/redemption engine ADR-0017 had deferred (`reward_entitlements`, `reward_entitlement_tokens`; `loyalty_programs` extended with `reward_type`/`reward_expiration_days`/`reward_repeatable`; see ADR-0018 and `docs/API_CONTRACTS.md` §16.11). TASK 13.2 built the POS benefit-application layer ADR-0018 had deferred: `loyalty_programs` further extended with `reward_benefit_type`/`reward_benefit_percentage_basis_points`/`reward_benefit_fixed_amount`; two new scope join tables `loyalty_program_reward_products`/`loyalty_program_reward_categories`; a new `sale_reward_usages` lifecycle table; `sale_discounts.source_type` widened to accept `'reward'` (see ADR-0019 and `docs/API_CONTRACTS.md` §16.12). Refund→reward reversal and Wallet-pass integration remain deferred (ADR-0018/ADR-0019's own Deferred sections).
...
- Rewards and memberships now reference `companies`, `customers`, `sales`, and immutable ledger identifiers exactly as anticipated here — see ADR-0017. Reward-entitlement issuance and redemption are now built (`reward_entitlements`, `reward_entitlement_tokens`; see ADR-0018). POS benefit application at checkout is now built too (`loyalty_programs.reward_benefit_type` and scope, `sale_reward_usages`, `sale_discounts.source_type='reward'`; see ADR-0019) — refund→reward reversal and Wallet-pass integration remain future extension points.
