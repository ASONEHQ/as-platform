...
**AS Rewards+ loyalty ledger** (the smallest coherent foundation — reward-entitlement issuance and redemption are deliberately deferred):
- `POST /api/v1/loyalty-programs` (`loyalty.manage`, Idempotency-Key), `GET /api/v1/loyalty-programs` (`loyalty.read`), `PUT /api/v1/loyalty-programs/{id}` (`loyalty.manage`, If-Match) — `{name*, active?, unit_type*: 'stamp'|'point', earn_quantity_per_sale?, minimum_sale_total?, reward_threshold?, reward_description?}`. A brand-new program defaults `active=false` even when the field is omitted — no company earns anything until it explicitly turns a program on.
