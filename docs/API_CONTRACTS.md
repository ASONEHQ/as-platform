...
Excluded: events/parties, advanced promotions/tax engines, invoicing, suppliers/purchasing, recipes, payroll, accounting, AI, advanced analytics, detailed WebSocket protocol, and offline authentication.

Customers, membership plans/entitlements, and the AS Rewards+ loyalty-ledger foundation are no longer excluded — see §16.10 and ADR-0017. Reward-entitlement issuance, loyalty redemption, and Wallet-pass integration remain excluded/deferred.
...
### 16.9 Promotions, discounts, and coupons engine (TASK 12.9)

§1's own "Excluded" list names "advanced promotions" as out of scope for this contract — this subsection is this task's own new addition, not a reconciliation against a pre-existing E-numbered contract the way refunds (§17.1) was. See ADR-0016 for the full design, including the exact pricing pipeline order, the priority/stacking algorithm, coupon concurrency, and the refund-compatibility fix this task required in `refunds.service.ts`.

- Implemented now: `POST /api/v1/promotions`, `GET /api/v1/promotions`, `GET /api/v1/promotions/{id}`, `PUT /api/v1/promotions/{id}` (`promotion.manage`/`promotion.read` — admin management: list/create/edit/activate, never a marketing analytics dashboard); `POST /api/v1/coupons`, `GET /api/v1/coupons`, `GET /api/v1/coupons/{id}`, `PUT /api/v1/coupons/{id}` (`coupon.manage`/`coupon.read`, same shape); `POST /api/v1/sales/pricing-quotes` (`sale.create` — a read-only preview: base subtotal, applied promotions/coupon(s), manual-discount preview, discount total, taxable subtotal, tax, grand total, per-line breakdown, and an honest rejection reason for any requested coupon code that didn't apply — never consumes a coupon redemption or mutates anything).
- `POST /api/v1/sales`'s existing body gained two purely optional fields: `coupon_codes` (string array) and `manual_discount` (`{scope: 'line'|'ticket', line_index?, type: 'percentage'|'fixed_amount', value, reason_code}`, requiring `discount.apply`) — omitting both reproduces the exact pre-TASK-12.9 request/response shape. The server independently re-derives every discount amount through the identical pricing engine the quote endpoint uses; an invalid/expired/exhausted coupon code fails the whole sale creation outright rather than silently completing without it.
- `GET /api/v1/sales/{id}/discounts` (`sale.read` — the same permission every other sale-detail read already requires) — a new read-only endpoint exposing the itemized `sale_discounts` breakdown for one sale (source type/id, label, reason code, amount, basis points), for Sale Detail's "exactly which commercial adjustments were applied" (Part V). Never mutates anything; an empty array is a legitimate answer for an undiscounted sale, never an error.
- Benefit types: `percentage`, `fixed_amount`, `fixed_price`, `quantity_nxm` (generalized NxM — "buy N, pay for M" — 2x1/3x2 are the `(2,1)`/`(3,2)` cases) for promotions; `percentage`/`fixed_amount` for coupons.
- Not implemented: a dedicated coupon-redemption endpoint (redemption is a side effect of sale creation, not a separate call), a promotion/coupon deletion route (deactivation via `PUT` covers the operational need), any customer-identity-dependent condition (age/membership/birthday/per-customer limit — a customer model now exists as of TASK 13.0/§16.10, but wiring the promotions rule engine to reference it is separate, not-yet-built future work), mixed-product NxM, and any combo/bundle benefit type (`products.product_type = 'kit'` stays reserved-but-unexploded, unchanged by this task).
- Permissions: `promotion.read`, `promotion.manage`, `coupon.read`, `coupon.manage`, `discount.apply` — five new codes, none overlapping an existing one.
- Realtime delivery: `promotion.created`/`promotion.updated`/`coupon.created`/`coupon.updated` outbox events are written (company-wide, no `branch_id`, since neither resource is branch-owned); no dedicated `discount.applied`/`coupon.redeemed` event exists — the fact is visible via the sale's own persisted `discount_total`/`sale_discounts` rows, not a special event. As with every other domain, no WebSocket publisher exists in this repository yet (§16.3).

### 16.10 Customers, memberships, and AS Rewards+ (TASK 13.0)

§1's own "Excluded" list previously named "customers, Rewards, ... memberships" as out of scope — this subsection is that approval. See ADR-0017 for the full design, including phone/email normalization, the duplicate/conflict policy, the membership activation boundary, renewal semantics, and the loyalty ledger's idempotency guarantees.

**Customers** (company-scoped, never branch-scoped):
- `POST /api/v1/customers` (`customer.create`, Idempotency-Key) — body `{first_name*, last_name?, display_name?, email?, phone?, birth_date?, notes?}`. `201` the created customer; `409 resource_conflict`/`customer_identity_conflict` on a duplicate normalized email/phone (never merges — the response carries the existing customer id(s) in `details` so a caller can offer them instead).
- `GET /api/v1/customers` (`customer.read`) — cursor-paginated, `search`/`status` filters. Success returns SUMMARY rows only (id, display name, status, version, created_at) — never phone/email/birth date in a list row.
- `GET /api/v1/customers/{id}` (`customer.read`) — the full record.
- `PATCH /api/v1/customers/{id}` (`customer.update`, If-Match) — any subset of the create fields plus `status` (`active`|`inactive`|`archived` — Part D18: no delete route exists or is planned; a customer with commercial history is archived, never destroyed).
- `POST /api/v1/customers/{id}/qr-tokens` (`customer.update`) — issues/rotates an opaque, cryptographically random QR identity token (revokes the prior active one in the same transaction); `GET /api/v1/customers/{id}/qr-tokens/active` (`customer.read`); `POST /api/v1/customers/qr-tokens/resolve` (`customer.read`, body `{token}`) — server-side lookup, rejects a token belonging to a different company. Never encodes email/phone/name; never a substitute for real authentication of a sensitive action.

**Sale integration**: `POST /api/v1/sales`'s existing body gained one purely optional field, `customer_id` — omitting it reproduces the exact pre-TASK-13.0 walk-in-sale behavior. `GET /api/v1/sales`/`GET /api/v1/sales/{id}` responses gained `customer_id`/`customer_display_name` (both `null` for a walk-in sale); `customer_display_name` is a frozen snapshot at sale-creation time, never re-read from today's (possibly since-edited) customer record. `GET /api/v1/sales` gained an optional `customer_id` query filter (Customer Detail's "recent sales," reusing the existing sales-history query, never a duplicate store).

**Membership plans** (the product/rule definition) and **customer memberships** (the issued entitlement — never the same table):
- `POST /api/v1/membership-plans` (`membership.manage`, Idempotency-Key), `GET /api/v1/membership-plans`, `GET /api/v1/membership-plans/{id}` (`membership.read`), `PUT /api/v1/membership-plans/{id}` (`membership.manage`, If-Match) — `{name*, description?, active?, product_id?, duration_days?, benefit_description?, branch_ids?}`. An optional `product_id` links a plan to a real sellable catalog product (`product_type = 'service'` in practice) — selling it through the ordinary `POST /sales`/cash-payment flow, with a `customer_id` attached, activates the membership automatically on payment settlement; no separate "buy membership" endpoint exists. Empty `branch_ids` means eligible at every branch.
- `GET /api/v1/customers/{customerId}/memberships` (`membership.read`); `POST /api/v1/customers/{customerId}/memberships` (`membership.issue`, Idempotency-Key) — manual/admin issuance only, body `{membership_plan_id*, starts_at?}`; `POST /api/v1/customer-memberships/{id}/renew` (`membership.issue`, Idempotency-Key) — creates a NEW entitlement row chained via `renewed_from_membership_id`, never mutates the original's `expires_at`; `POST /api/v1/customer-memberships/{id}/cancel` (`membership.manage`, If-Match, body `{reason*}`).
- `POST /api/v1/memberships/validate` (`membership.read`, body `{customer_id*, branch_id*}`) — the ONE server-authoritative validity decision (`{valid, reason, eligible_branch, membership}`); a client never computes membership validity itself from `status`/`expires_at`.
- Activation boundary: a POS-purchased membership is inserted directly as `active`, exactly once, at the exact moment the originating Sale newly settles (never at sale-creation time — "no activation before payment" is structural, not a runtime check). A refund against a sale that activated a membership never automatically revokes it (no canonical commercial rule for that exists); an administrator may separately cancel it via the route above.

**AS Rewards+ loyalty ledger** (the smallest coherent foundation; reward-entitlement issuance/redemption now built on top of it — see §16.11 — and checkout benefit application built on top of that — see §16.12):
- `POST /api/v1/loyalty-programs` (`loyalty.manage`, Idempotency-Key), `GET /api/v1/loyalty-programs` (`loyalty.read`), `PUT /api/v1/loyalty-programs/{id}` (`loyalty.manage`, If-Match) — `{name*, active?, unit_type*: 'stamp'|'point', earn_quantity_per_sale?, minimum_sale_total?, reward_threshold?, reward_description?, reward_type?: 'vip_pass', reward_expiration_days?, reward_repeatable?}` (the last three fields: TASK 13.1, see §16.11). A brand-new program defaults `active=false` even when the field is omitted — no company earns anything until it explicitly turns a program on. `reward_threshold` and `reward_type` must be set together (400 `validation_error` otherwise).
- `GET /api/v1/customers/{customerId}/loyalty` (`loyalty.read`) — `{account: {id, status}|null, balances: [{program_id, unit_type, balance}], ledger: [...]}`. Balance is always derived (`SUM(quantity)` over the append-only ledger), never a cached counter; `account: null` is the honest answer for a customer who has never earned or been adjusted.
- `POST /api/v1/customers/{customerId}/loyalty/adjust` (`loyalty.adjust` — separately permissioned from `loyalty.manage`, Idempotency-Key, body `{quantity*, unit_type*, reason*, loyalty_program_id?}`) — a signed manual correction, always carrying a reason and the acting admin.
- Automatic earning: exactly one `'earn'` ledger entry per (active program, qualifying completed sale with a customer attached, at/above the program's own `minimum_sale_total` if set) — never every sale unconditionally, never for a walk-in sale, and never twice for a retried/replayed settlement (a real unique constraint on `(company_id, loyalty_program_id, source_type, source_id)` guarantees this at the database).

**Permissions**: `customer.read`, `customer.create`, `customer.update`, `membership.read`, `membership.manage`, `membership.issue`, `loyalty.read`, `loyalty.manage`, `loyalty.adjust` — nine new codes, none overlapping an existing one. (§6's own registry table predates TASK 12.9/13.0 and was not extended by either — `packages/database/src/seeds/technical-permissions.ts` is the current source of truth for the full permission set.)

**Realtime delivery**: `customer.created`/`customer.updated`, `customer.qr_token.issued`, `membership_plan.created`/`updated`, `membership.activated`/`membership.cancelled`, `loyalty_program.created`/`updated`, `loyalty.earned`/`loyalty.adjusted` outbox events are written (company-wide, no `branch_id`); every payload carries only ids and non-sensitive status fields, never a name/email/phone/birth date. As with every other domain, no WebSocket publisher exists in this repository yet (§16.3).

### 16.11 Reward entitlements, redemption, and VIP Pass (TASK 13.1)

§16.10 named reward-entitlement issuance and redemption as deliberately deferred; this subsection is that work. See ADR-0018 for the full design, including the threshold-cycle issuance math, the idempotent-issuance database guarantee, the lazy-expiry model, and the explicit deferral of POS benefit application.

**Entitlements** (company-scoped, always resolved through a customer):
- `GET /api/v1/customers/{customerId}/reward-entitlements` (`reward.read`) — every entitlement for that customer (available, redeemed, expired, revoked — never only the active ones; Customer Detail shows full history).
- `POST /api/v1/customers/{customerId}/reward-entitlements` (`reward.issue`, Idempotency-Key) — manual/admin issuance only, body `{loyalty_program_id*, reason_code*, expires_at?}`. The reward type is always read from the program's own configuration, never supplied by the caller.
- `GET /api/v1/reward-entitlements/{id}` (`reward.read`) — a single entitlement; carries an `ETag` (`version`) for the If-Match routes below.
- `POST /api/v1/reward-entitlements/{id}/redeem` (`reward.redeem`, Idempotency-Key, optional body `{branch_id?}`) — the ONE server-authoritative redemption decision. `200` the now-redeemed entitlement; `404` wrong company or not found; `409 reward_already_redeemed`/`reward_not_available`/`reward_expired` (a stale `available` row past its `expires_at` is lazily and durably transitioned to `expired` — committed to the database, not merely computed for this one response — then rejected; a replayed same-key attempt against the now-`expired` row rejects identically every time).
- `POST /api/v1/reward-entitlements/{id}/revoke` (`reward.revoke`, If-Match, body `{reason*}`) — `200` the revoked entitlement; revoking an already-revoked entitlement is a no-op returning the current row; revoking an already-redeemed entitlement is rejected (`409 reward_already_redeemed`).

Every entitlement response carries both the raw persisted `status` and a live-computed `effective_status` (`available`/`redeemed`/`expired`/`revoked`) — a client MUST key every "is this redeemable right now" decision off `effective_status`, never off `status` alone and never off a locally recomputed expiry.

**Presentation tokens** (a distinct concept from a customer's own QR identity — §16.10's `customer_qr_tokens` — never conflated):
- `POST /api/v1/reward-entitlements/{id}/presentation-token` (`reward.read`) — issues/rotates an opaque, cryptographically random token for ONE entitlement (revokes the prior active one in the same transaction).
- `GET /api/v1/reward-entitlements/{id}/presentation-token/active` (`reward.read`) — the currently active token, or `null`.
- `POST /api/v1/reward-entitlements/resolve-token` (`reward.read`, body `{token*}`) — server-side lookup; rejects a token belonging to a different company, never leaks which company a token belongs to. Never encodes customer PII or reward metadata; not authentication.

**Automatic issuance**: exactly one entitlement per (active reward-bearing program, customer, threshold-cycle) — `cycle_number = floor(cumulative_earned_units / reward_threshold)`, recomputed fresh from the loyalty ledger on every evaluation, never a stored counter. Fires inside the same post-settlement transaction as loyalty earning (§16.10) — structurally never before settlement, never on a failed/pending sale, never for an inactive program. A real database unique constraint (`company_id, loyalty_account_id, loyalty_program_id, cycle_number`) makes a duplicate issuance for the same cycle impossible, proven under real concurrent-request testing, not merely at the service layer. `reward_repeatable=false` issues at most one entitlement per program per account, ever, regardless of how many further cycles are crossed.

**Accounting**: issuing an entitlement never deletes, resets, or mutates any `loyalty_ledger` row and never decrements the derived balance (§16.10) — the ledger keeps counting up forever; the entitlement history is the separate durable record of what was actually issued.

**Deliberately deferred at the time of TASK 13.1** (ADR-0018): applying a redeemed entitlement's actual commercial benefit at checkout was not yet automatic. TASK 13.2 (§16.12) built exactly that. A refund against a sale that contributed toward a reward crossing still never automatically revokes the entitlement or reverses the ledger entry (unchanged by §16.12).

**Permissions**: `reward.read`, `reward.redeem`, `reward.issue`, `reward.revoke` — four new codes. A cashier is granted `reward.read` + `reward.redeem` only, mirroring `discount.apply`'s/`membership.issue`'s established checkout-action-vs-admin-action separation (§6/`packages/database/src/seeds/technical-permissions.ts` remains the source of truth for the full set).

**Realtime delivery**: `reward.issued` (both automatic-threshold and manual issuance), `reward.redeemed`, `reward.expired` (the lazy-expiry transition, once it's actually discovered and persisted), `reward.revoked`, `reward.token_issued` outbox events are written (branch-scoped when a branch context exists, e.g. redemption/expiry; company-wide otherwise); every payload carries only ids and status, never PII, never a raw presentation-token value. As with every other domain, no WebSocket publisher exists in this repository yet (§16.3).

### 16.12 Reward benefit application and VIP Pass POS consumption (TASK 13.2)

§16.11 named POS benefit application as deliberately deferred; this subsection is that work. See ADR-0019 for the full design, including the pricing-pipeline placement, the settlement-only consumption mechanism and its reuse of §16.11's own redemption row lock, the zero-total settlement path, and the refund/receipt interaction.

**Benefit configuration** (an extension of `loyalty_programs`, §16.10): `POST /api/v1/loyalty-programs`/`PUT /api/v1/loyalty-programs/{id}` gain purely optional fields — `reward_benefit_type?: 'percentage_discount'|'fixed_amount_discount'|'fixed_price'|'free_eligible_item'`, `reward_benefit_percentage_basis_points?`, `reward_benefit_fixed_amount?`, `reward_scope_product_ids?: string[]`, `reward_scope_category_ids?: string[]`. Omitting all five reproduces the exact pre-TASK-13.2 program (tracks progress, issues entitlements, grants no checkout benefit). Setting `reward_benefit_type` requires at least one non-empty scope array (400 `validation_error` otherwise — a benefit with nothing to apply to is rejected outright, never silently accepted as "applies to everything"). On update, omitting BOTH scope arrays leaves the existing scope untouched; supplying either (even `[]`) replaces the scope explicitly.

**Checkout integration** (both read the reward exactly the same way — see ADR-0019 D5):
- `POST /api/v1/sales/pricing-quotes` (§16.9) gains two purely optional fields, `customer_id?`/`reward_entitlement_id?` (required together) — the response's existing per-line/applied-discount breakdown now may include a `source_type: 'reward'` entry. Read-only: no quote, however many times repeated, ever changes any reward entitlement's state.
- `POST /api/v1/sales` (§16.9) gains one purely optional field, `reward_entitlement_id?` (requires `customer_id` also be present; `reward.redeem` permission). The server re-validates the entitlement fresh — a prior quote result is never trusted. If the pricing engine's cascade actually produces a reward discount, a `sale_reward_usages` row is written (`status='applied'`) — the entitlement itself is NOT yet touched; a pending-payment sale has not consumed anything.
- `POST /api/v1/sales/{sale_id}/zero-total-completion` (`reward.redeem`, Idempotency-Key not required — see ADR-0019 D9) — settles a `pending_payment` sale whose `total` is exactly `0` with NO payment/payment-attempt/cash-movement row created at all; `400` if the total is not zero or the sale is not `pending_payment`. Response shape matches the existing sale-receipt composition (§16.7).
- `GET /api/v1/sales/{id}/discounts` (§16.9) may now include a `source_type: 'reward'` row (`source_id` = the reward entitlement id, `label` = a generic, non-PII display string) — no new endpoint was needed; the existing generic discount listing already surfaces it.

**Settlement-only consumption**: the reward entitlement is marked `redeemed` (§16.11) and its `sale_reward_usages` row transitions `applied → consumed` inside the exact same database transaction that settles the sale — never earlier. Cancelling a `pending_payment` sale that had a reward attached (`POST /api/v1/sales/{id}/cancel`, unchanged route) transitions that sale's usage row `applied → released` and leaves the entitlement completely untouched (never "un-redeemed", since it was never redeemed). Two sales racing the same entitlement's settlement resolve to exactly one winner (proven under real concurrent-request testing, reusing §16.11's own entitlement row lock) — the loser's payment attempt fails cleanly and its sale is never marked `completed`.

**Not implemented**: a dedicated Customer-Detail "this entitlement was redeemed by sale X" read endpoint (the link is durably recorded in `sale_reward_usages` and queryable, just not yet surfaced through a new response shape); any reward `combinable_with_*` policy flags (a reward stacks with promotions/coupons/manual discounts purely through the pricing engine's existing cascading-remainder mechanism, §16.9 — no explicit combinability schema exists); any automatic reward reactivation on refund (unchanged from §16.11 — a refund never touches `reward_entitlements`/`sale_reward_usages`).

**Permissions**: no new permission codes — `reward.redeem` (§16.11) is reused for both attaching a reward at checkout and completing a zero-total sale.

**Realtime delivery**: `reward.redeemed` (§16.11, now also fired from the settlement path, carrying the settling `sale_id` in its payload) and a new `sale.completed_without_payment` outbox event (branch-scoped, sale id only) for the zero-total path — written alongside, never instead of, the sale's own normal `sale.completed` event. No new event type exists for "a reward was attached to a sale" — that fact is visible via the sale's own persisted `sale_discounts`/`sale_reward_usages` rows, not a special event, matching §16.9's identical stance on discount application.
...
### 17.1 Returns/refunds engine implementation (TASK 12.8)

E081/E082/E083/E084/E086 are now implemented — the complete post-sale correction lifecycle (full/partial return, refund calculation from the original sale's own frozen snapshot, refund execution, inventory restoration, cash-drawer impact, Sales History visibility). See ADR-0015 for the full design, including the exact cumulative refund-limit safety, cash/card refund semantics, and idempotency/concurrency guarantees.

- Implemented now: `GET /api/v1/sales/{sale_id}/refundable-balance` (E081), `POST /api/v1/refunds` (E082 — request body is `{id?, sale_id*, reason_code*, reason_note?, items: [{sale_item_id*, quantity*}]}`; no client-submitted amount/method/disposition field of any kind — the server independently recalculates every value from the original sale item's frozen snapshot and derives `refund_method`/`restock_disposition` itself), `GET /api/v1/refunds/{id}` (E083), `GET /api/v1/refunds` (E084 — also what a Sale Detail's "returns for this sale" section calls, via its `sale_id` filter; no separate/duplicate endpoint was built for that), `POST /api/v1/refunds/{id}/completion` (E086 — the atomic effects boundary: payment reversal, cash-drawer movement, inventory restoration, all one transaction).
- Not implemented: E085 (a dedicated separate-approval endpoint) and E087 (refund cancellation). This implementation instead requires the same actor creating a refund to also hold `refund.approve` in order to self-approve immediately (`requested → approved` in one step, a real transition in §21.4's own diagram); an actor without it is rejected outright with `refund_approval_required`, never silently queued into an unreachable `pending_approval` state. See ADR-0015 D7.
- Refund method always follows the original sale's captured payment method — there is no request field to choose a different one (ADR-0015 D9). A partial refund of a `card_terminal` payment is rejected outright (`payment_not_reversible`); only a full-amount card refund is ever attempted, and only when Mercado Pago is actually configured — which it is not in any environment this task touches, so that path is proven to fail honestly, never to simulate success (ADR-0015 D15).
- Refund permissions are the five already-reserved codes from the technical permission catalog (`refund.read`, `refund.create`, `refund.approve`, `refund.complete`, `refund.cancel`) — no new permission was introduced; `refund.cancel` remains seeded but unused (no cancellation endpoint exists).
- No dedicated refund-receipt endpoint was built — `GET /refunds/{id}` (E083) already returns everything a return/refund receipt needs; Flutter composes it client-side the same way it already composes the sale receipt (ADR-0015 D17).
- `GET /api/v1/sales` (§16.8) and `GET /api/v1/sales/{id}` now additionally expose a derived, read-only `refund_state` field (`not_refunded | partially_refunded | fully_refunded`), computed purely from completed `refunds`/`refund_items` rows — `sales.status` itself is never overwritten or hidden by this (ADR-0015 D14).
- Realtime delivery: the already-reserved `refund.approved` and `refund.completed` outbox events (§11.7 of REALTIME_EVENTS.md) are now written — `refund.approved` once, at creation (since a self-approved refund never observably passes through a separate `requested` state, `refund.requested` is not additionally published for the same command), and `refund.completed` at E086. `refund.cancelled` is never published (no cancellation path exists — E087 above). As with every other domain, no WebSocket publisher exists in this repository yet (§16.3) — a client must poll `GET /refunds/{id}` for status.
