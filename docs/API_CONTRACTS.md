...
**AS Rewards+ loyalty ledger** (the smallest coherent foundation; reward-entitlement issuance and redemption now built on top of it — see §16.11):
...
**Deliberately deferred** (ADR-0018): applying a redeemed entitlement's actual commercial benefit at checkout (waiving/discounting a line item) is NOT automatic — redemption records the fact only; a cashier applies whatever discount the existing checkout flow already supports (e.g. `discount.apply`) manually today. A refund against a sale that contributed toward a reward crossing never automatically revokes the entitlement or reverses the ledger entry.
