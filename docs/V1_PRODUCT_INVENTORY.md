...
| Reports (Sales/Financial/Inventory/Customers/Employees/Parties/Access) | 1 | ✅ (`apps/api/src/modules/reports/` — 7 real server-side-aggregated report areas, real date-range+branch scoping, CSV export on Sales/Financial; financial report reconciles bit-for-bit against the real `CashService.summary()` fold logic) | ✅ | ✅ (`pos_reports_screen.dart`, a tabbed per-area screen) | ✅ (`report.read`) | ✅ (17 backend + 12 Flutter tests) | 🟢 GREEN | **TASK 14.4 (Wave 2) built this for real** — the first real metrics UI in the current platform, in addition to (not a replacement for) the still-real Sales History / Cash Session Summary / Refunds list / Inventory Balances screens the launch already relies on. Deliberately does NOT reproduce the legacy's own 2 admitted-fake fields (`prom_estancia`=95, always-0 occupancy). Remaining gap: no single consolidated "today at a glance" dashboard screen (sales trend + parties + alerts on one view) |
...
- The 7-area Reports screen (TASK 14.4 Wave 2, real and GREEN — existing
  screens already cover the minimum operational need for launch; Reports
  is an operational convenience, not a Sept 15 blocker) and a single
  consolidated "today at a glance" dashboard screen (still missing)
